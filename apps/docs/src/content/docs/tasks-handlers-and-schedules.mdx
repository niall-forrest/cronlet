...
Tool handlers execute steps linearly within a single run. Retries are currently run-level, so failed runs restart from the beginning rather than resuming from the failed step.
