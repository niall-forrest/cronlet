...
description: Method-by-method reference for CloudClient, tool exports, errors, and real payload shapes.
sidebar:
  order: 7
  label: SDK API Reference
---

This page mirrors the current `@cronlet/sdk` package surface.
...
- `tasks.create()` and `tasks.patch()` use typed object schedules.
- Tool definitions mention string schedules, but the handler does not currently parse them.
